...
while not terminated:
    ...
    velocity_gt = np.append(
        velocity_gt, info["rocket_states"][3:6].reshape(-1, 1), axis=1
    )
    # estimated velocity
    velocity_est = np.append(velocity_est, agent.states[3:6].reshape(-1, 1), axis=1)
    # ground truth attitude
    attitude_gt = np.append(
        attitude_gt, info["rocket_states"][6:10].reshape(-1, 1), axis=1
    )
    # estimated attitude
    attitude_est = np.append(attitude_est, agent.states[6:10].reshape(-1, 1), axis=1)

    print(
        f"simulation_time: {observation['simulation_time']:.2f} sec, total reward: {info['popped_count']:.2f}",
        end="\r",
    )
