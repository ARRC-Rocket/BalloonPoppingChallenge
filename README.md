...
4. Upload your .json file generated in [/results](BalloonPoppingGymEnv\evaluation\results) folder to the [leaderboard](https://balloonpoppingchallenge.arrcrocket.org/). The file is plain JSON (it holds your `team_secret` in the clear, so treat it as a credential), not an encrypted archive.
